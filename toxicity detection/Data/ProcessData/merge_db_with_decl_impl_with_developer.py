...
合并规则：
1) 以 DB 为主表。
2) 从 CSV 中提取 domain、declaration_atomic_ids、implementation_atomic_ids。
3) 其中 domain 以 CSV 为准。
4) declaration_atomic_ids 和 implementation_atomic_ids 也以 CSV 为准。
5) 其他重复字段一律以 DB 为准。
6) 默认使用 inner join，只保留 DB 和 CSV 都存在的 Skill。
7) 可选按 source_plat 一起作为联合键；若开启但当前文件缺失该列，可自动回退为仅按 skill 键合并。
8) 在原有 skill/source_plat 匹配基础上，额外加入 developer 作为联合键；developer 为 NULL 时也作为一个可匹配的键值。

当前默认适配：
- DB 表: skills
- DB 中 skill 键列: name
- CSV 中 skill 键列: skill_id
- CSV 中补充列: domain, declaration_atomic_ids, implementation_atomic_ids
"""
...
CONFIG = {
    ...
    # 可选联合键：source_plat
    # 注意：这里不做 source_plat/source_plt 等别名兼容，仍沿用原脚本字段名配置。
    ...
    # 新增联合键：developer
    # developer 会参与 DB 与 CSV 的匹配。
    # 如果 developer 为 NULL/空值，会被统一视为一个普通键值参与匹配。
    "use_developer": True,
    "db_developer_col":  "developer",
    "csv_developer_col": "developer",
    "developer_null_value": "__NULL_DEVELOPER__",

    # 从 CSV 中提取这些列补到 DB 中
    # 注意：domain 这一列最终以 CSV 为准
    "csv_domain_col": "domain",
...
def resolve_join_keys(
    ...
    if CONFIG["use_source_plat"]:
        ...
        if db_has and csv_has:
            ...
            raise KeyError(
                "开启了 source_plat 联合键，但缺少字段: " + ", ".join(missing)
            )

    if CONFIG["use_developer"]:
        db_dev = CONFIG["db_developer_col"]
        csv_dev = CONFIG["csv_developer_col"]

        # developer 是本次新增的必需联合键：缺失时直接报错，不做静默回退。
        ensure_column_exists(db_df, db_dev, "DB")
        ensure_column_exists(csv_df, csv_dev, "CSV")

        db_join_keys.append(db_dev)
        csv_join_keys.append(csv_dev)
        developer_enabled = True

    return db_join_keys, csv_join_keys, source_plat_enabled, developer_enabled


def build_csv_subset(
    csv_df: pd.DataFrame,
    csv_join_keys: List[str]
) -> pd.DataFrame:
    """
    从 CSV 中只保留：
    1) join keys
    2) domain
    3) declaration_atomic_ids
    4) implementation_atomic_ids
    """
    domain_col = CONFIG["csv_domain_col"]
    decl_col = CONFIG["csv_declaration_col"]
    impl_col = CONFIG["csv_implementation_col"]

    ensure_column_exists(csv_df, domain_col, "CSV")
...
def normalize_developer_key(series: pd.Series) -> pd.Series:
    """
    将 developer 连接键规范化为字符串。

    关键点：
    - 真实 NULL/NaN/None 统一映射到 developer_null_value；
    - 常见字符串形式的空值，例如 "NULL"、"nan"、"None"、空字符串，也统一映射；
    - 其他 developer 值转成去除首尾空格后的字符串。

    这样 developer 为 NULL 时，不会被丢弃，而是作为一个普通键值参与匹配。
    """
    null_token = CONFIG["developer_null_value"]
    null_like_strings = {"", "null", "none", "nan", "na", "n/a"}
...
def normalize_general_key(series: pd.Series) -> pd.Series:
    """
    将普通连接键统一转成字符串，避免数值/字符串混型导致 join 失败。
    """
...
def normalize_key_columns(
    ...
    db_join_keys: List[str],
    csv_join_keys: List[str]
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    把连接键统一转成字符串，避免数值/字符串混型导致 join 失败。

    developer 是特殊情况：NULL 也要作为一个可匹配的键值。
    """
    db_df = db_df.copy()
    ...
    for col in db_join_keys:
        if CONFIG["use_developer"] and col == db_dev:
            db_df[col] = normalize_developer_key(db_df[col])
        else:
            db_df[col] = normalize_general_key(db_df[col])

    for col in csv_join_keys:
        if CONFIG["use_developer"] and col == csv_dev:
            csv_subset[col] = normalize_developer_key(csv_subset[col])
        else:
            csv_subset[col] = normalize_general_key(csv_subset[col])

    return db_df, csv_subset


def merge_db_with_csv_subset(
    db_df: pd.DataFrame,
    csv_subset: pd.DataFrame,
    db_join_keys: List[str],
    csv_join_keys: List[str]
    ...
    注意：
    - domain 最终以 CSV 为准。
    - declaration_atomic_ids 和 implementation_atomic_ids 来自 CSV。
    - 其他字段保持 DB 为准。
    """
    domain_col = CONFIG["csv_domain_col"]
    ...
    # 如果左右键列名不同，删掉来自 CSV 的键列；
    # 如果同名，pandas 不会额外生成重复键列。
    extra_csv_key_cols = [
        k for k in csv_join_keys
        if k not in db_join_keys and k in merged.columns
    ]
    if extra_csv_key_cols:
        merged = merged.drop(columns=extra_csv_key_cols)

    # 核心修改：
    # 如果 DB 和 CSV 都有 domain，则 merge 后会出现 domain 和 domain_csv。
    # 这里用 CSV 中的 domain 覆盖 DB 中的 domain。
    csv_domain_col = f"{domain_col}_csv"
    if csv_domain_col in merged.columns:
        merged[domain_col] = merged[csv_domain_col]
        merged = merged.drop(columns=[csv_domain_col])
...
def reorder_output_columns(
    ...
    original_db_columns: List[str]
) -> pd.DataFrame:
    """
    输出列顺序：
    1) 原 DB 列顺序
    2) declaration_atomic_ids
    3) implementation_atomic_ids
    4) 其他意外残留列
    """
    decl_col = CONFIG["csv_declaration_col"]
    impl_col = CONFIG["csv_implementation_col"]
...
def main() -> None:
    ...
    db_join_keys, csv_join_keys, source_plat_enabled, developer_enabled = resolve_join_keys(
        db_df,
        csv_df
    )
    ...
    db_df, csv_subset = normalize_key_columns(
        db_df,
        csv_subset,
        db_join_keys,
        csv_join_keys
    ...
    merged_df = merge_db_with_csv_subset(
        db_df,
        csv_subset,
        db_join_keys,
        csv_join_keys
    ...
    print(f"developer enabled: {developer_enabled}")
    print(f"Merge how: {CONFIG['merge_how']}")
    print(f"Output rows: {len(merged_df)}")
    print(f"Output file: {CONFIG['output_csv']}")

    domain_col = CONFIG["csv_domain_col"]
    if domain_col in merged_df.columns:
        print(f"Domain column source: CSV.{domain_col}")

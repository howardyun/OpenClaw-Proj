...
CONFIG: Dict[str, Any] = {
    "input_csv": "pdei_demo_input_domain_code.csv",
    "out_dir": "output/pdei_outputs",
    "output_prefix": "pdei",
    "top_n": 100,
...
def build_rename_map(column_map: Mapping[str, str]) -> Dict[str, str]:
    return {actual: standard for standard, actual in column_map.items()}


def standardize_columns(df: pd.DataFrame, column_map: Mapping[str, str]) -> pd.DataFrame:
    rename_map = build_rename_map(column_map)
    missing_actual = [actual for actual in rename_map if actual not in df.columns]
    if missing_actual:
        raise ValueError(f"输入 CSV 缺少这些列: {missing_actual}")
    out = df.rename(columns=rename_map).copy()
    return out


def validate_required_columns(df: pd.DataFrame) -> None:
    missing = [col for col in REQUIRED_STANDARD_COLUMNS if col not in df.columns]
    if missing:
        raise ValueError(f"标准化后缺少必需列: {missing}")
...
def score_one_skill(row: pd.Series, config: Mapping[str, Any]) -> pd.Series:
    requested_atoms = parse_atom_set(row["requested_atoms"])
    necessary_atoms = parse_atom_set(row["necessary_atoms"])

    reach_result = compute_reach(
        skill_download_count=normalize_int(row["skill_download_count"]),
        skill_star_count=normalize_int(row["skill_star_count"]),
        skill_fork_count=normalize_int(row["skill_fork_count"]),
        developer_github_stars=normalize_int(row["developer_github_stars"]),
        developer_is_org=normalize_bool(row["developer_is_org"]),
...
def build_path_trigger_stats(df: pd.DataFrame, top_n: int = 3) -> Dict[str, Dict[str, Any]]:
    stats: Dict[str, Dict[str, Any]] = {}
    for path_col in ["path_A", "path_B", "path_C", "path_D", "path_E"]:
        triggered = df[df[path_col] == 1].copy()
        top_cases = []
        if not triggered.empty:
            cols = [c for c in ["skill_id", "skill_name", "domain", "pdei_score"] if c in triggered.columns]
            top_cases = triggered.nlargest(top_n, "pdei_score")[cols].to_dict(orient="records")
        stats[path_col] = {
...
def export_all_outputs(scored_df: pd.DataFrame, config: Mapping[str, Any]) -> Dict[str, Path]:
    ...
    domain_col = str(config["domain_col"])
    ...
    path_stats = build_path_trigger_stats(scored_df)
...
def prepare_input_dataframe(config: Mapping[str, Any]) -> pd.DataFrame:
    raw_df = load_input_csv(config["input_csv"])
    standardized_df = standardize_columns(raw_df, config["column_map"])
    validate_required_columns(standardized_df)
    return standardized_df


def run_pdei_pipeline(config: Mapping[str, Any]) -> Dict[str, Path]:
    input_df = prepare_input_dataframe(config)
    scored_df = score_all_skills(input_df, config)
